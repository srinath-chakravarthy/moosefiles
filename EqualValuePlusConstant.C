...
registerMooseObject("TheronApp", EqualValuePlusConstant);
...
EqualValuePlusConstant::computeQpResidual(Moose::ConstraintType type)
{
  ...
  {
    case Moose::Primary:
      return (_u_primary[_j] - _u_secondary[_i] + _func.value(_t, *_var.node())) * _penalty;
    case Moose::Secondary:
      return (_u_secondary[_i] - _u_primary[_j] - _func.value(_t, *_var.node())) * _penalty;
  }
  return 0.;
}
